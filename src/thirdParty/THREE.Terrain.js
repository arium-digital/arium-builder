 ...
 *     `options.ySegments + 1`.
 */
export const generateTerrain = function (options) {
  var defaultOptions = {
    after: null,
...
Terrain.toHeightmap = function (g, options) {
  ...
  for (var row = 0; row < rows; row++) {
    for (var col = 0; col < cols; col++) {
      var i = row * cols + col,
        idx = i * 4;
      data[idx] = data[idx + 1] = data[idx + 2] = Math.round(
        ((g[i * 3 + 2] - min) / spread) * 255
      );
      data[idx + 3] = 255;
    }
  }
  ...
  for (
    ...
    for (var j = 0; j < yl; j++) {
      ...
      if (neighborKeys.length % 2 === 1) {
        ...
        median =
          (g[neighborKeys[halfKey - 1]] + g[neighborKeys[halfKey]]) * 0.5;
      }
...
Terrain.MultiPass = function (g, options, passes) {
  ...
  for (var i = 0, l = passes.length; i < l; i++) {
    var amp =
        typeof passes[i].amplitude === "undefined" ? 1 : passes[i].amplitude,
...
Terrain.Curve = function (g, options, curve) {
  var range = (options.maxHeight - options.minHeight) * 0.5,
    scalar =
      options.frequency / (Math.min(options.xSegments, options.ySegments) + 1);
  for (
    var i = 0, xl = options.xSegments + 1, yl = options.ySegments + 1;
    i < xl;
    i++
...
Terrain.DiamondSquare = function (g, options) {
  ...
  var segments = THREE.Math.ceilPowerOfTwo(
    ...
    for (var i = 0, xl = options.xSegments + 1; i < xl; i++) {
      for (var j = 0, yl = options.ySegments + 1; j < yl; j++) {
        ...
        if (distance > smoothDistance) {
          ...
          g[j * xl + i] +=
            Math.cos((distance / smoothDistance) * Math.PI * 2) * displacement;
        }
      }
    }
  }
...
Terrain.Perlin = function (g, options) {
  ...
  var range = (options.maxHeight - options.minHeight) * 0.5,
    divisor =
      (Math.min(options.xSegments, options.ySegments) + 1) / options.frequency;
  for (var i = 0, xl = options.xSegments + 1; i < xl; i++) {
    for (var j = 0, yl = options.ySegments + 1; j < yl; j++) {
    ...
    for (var j = 0, yl = options.ySegments + 1; j < yl; j++) {
      ...
      for (var k = 0; k < 20; k++) {
        var x =
          Math.pow(1 + r11, -k) *
          Math.sin(
            Math.pow(1 + r12, k) * (i + 0.25 * Math.cos(j) + r14 * j) * r13
          );
        var y =
          Math.pow(1 + r21, -k) *
          Math.sin(
            Math.pow(1 + r22, k) * (j + 0.25 * Math.cos(i) + r24 * i) * r23
          );
...
Terrain.ScatterMeshes = function (geometry, options) {
  if (!options.mesh) {
    console.error(
      "options.mesh is required for Terrain.ScatterMeshes but was not passed"
    );
    return;
  }
  if (!options.scene) {
  ...
  for (var i = 0; i < geometry.attributes.position.array.length; i += 9) {
    ...
    if (spreadIsNumber) {
      ...
      if (rv < options.spread) {
        ...
        place =
          Terrain.EaseInOut((rv - options.spread) * spreadRange) *
            options.spread >
          Math.random();
      }
    } else {
...
Terrain.Influences = {
  ...
  Volcano: function (x) {
    return (
      0.94 -
      0.32 * (Math.abs(2 * x) + Math.cos(2 * Math.PI * Math.abs(x) + 0.4))
    );
  },
};
...
Terrain.Influence = function (g, options, f, x, y, r, h, t, e) {
  ...
  for (var i = xs; i < xe; i++) {
    for (var j = ys; j < ye; j++) {
      ...
      // jscs:ignore requireSpaceAfterKeywords
